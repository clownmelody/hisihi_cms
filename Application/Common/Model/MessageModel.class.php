...
class MessageModel extends Model
{
     ...
     * @auth 陈一枭
     */
    public function sendMessage($to_uid, $content = '', $title = '您有新的消息', $url, $from_uid = 0, $type = 0, $appname = '', $apptype = '', $source_id = 0, $find_id = 0, $hide=0)
    {
        if ($to_uid == is_login()) {
            return 0;
        }
        //调用极光推送
        //$Addons = A("Addons://JPush/JPush");
        //$Addons->pushtouser($user,$content,$title,null);

        $this->sendMessageWithoutCheckSelf($to_uid, $content, $title, $url, $from_uid, $type, $appname, $apptype, $source_id, $find_id, $hide);
    }
     ...
     * @param int $type 消息类型，0系统，1用户，2应用
     */
    public function sendMessageWithoutCheckSelf($to_uid, $content = '', $title = '您有新的消息', $url, $from_uid = 0, $type = 0, $appname = '', $apptype = '', $source_id = 0, $find_id = 0, $hide=0)
    {
