...
function query_user($fields = null, $uid = null)
{
    ...
    if (in_array('name', $fields)) {
        $ucenterResult['name'] = op_t($ucenterResult['nickname']);
    }
    ...
    if (in_array('icons_html', $fields)) {
        ...
        foreach ($iconUrls as $e) {
            $result['icons_html'] .= "<img src=\"{$e}\" width=\"20px\" title=\"对方已绑定手机\"/>";
        }
    ...
    if(in_array('group', $fields)) {
        $profile_group = _profile_group($uid);
        $result['group'] = $profile_group['gid'];
    }

    //扩展信息
    if(in_array('extinfo', $fields)) {
        $profile_group = _profile_group($uid);
        $info_list = _info_list($profile_group['id'], $uid);
        $result['extinfo'] = $info_list;
    }
    ...
    foreach ($result as $field => $value) {
        ...
        if (!in_array($field, array('rank_link', 'icons_html', 'space_link', 'expand_info', 'extinfo'))) {
...
function _info_list($id = null, $uid = null)
{
    $info_list = null;
