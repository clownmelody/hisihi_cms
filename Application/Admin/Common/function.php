	...
	if(!empty($grid['href'])){
        ...
        foreach($links as $link){
            ...
            if(preg_match('/^\[([a-z_]+)\]$/',$href,$matches)){
                ...
                // 替换系统特殊字符串
                $href	=	str_replace(
                    array('[DELETE]','[TOPEDIT]','[EDIT]','[MODEL]'),
                    array('del?ids=[id]&model=[MODEL]','topedit?id=[id]&model=[MODEL]','edit?id=[id]&model=[MODEL]',$model['id']),
                    $href);
 ...
 *
 */
function int_to_string(&$data,$map=array('status'=>array(1=>'正常',-1=>'删除',0=>'禁用',2=>'未审核',3=>'草稿',4=>'推荐'))) {
