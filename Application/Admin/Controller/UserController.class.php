...
class UserController extends AdminController
{
    ...
    public function index()
    {
        ...
        foreach ($list as &$user) {
            $member = M('UcenterMember')->where('id=' . $user['uid'])->field('mobile')->find();
            $user['mobile'] = $member['mobile'];
            $group = M('AuthGroupAccess')->where('uid=' . $user['uid'])->field('group_id')->find();
            if($group['group_id'] == 5)
                $user['group'] = '设计师';
            else if($group['group_id'] == 6)
                $user['group'] = '讲师';
            else if($group['group_id'] == 7)
                $user['group'] = '后台管理员';
        }
        int_to_string($list);
...
str;
    ...
    public function expandinfo_details($uid = 0)
    {
        ...
        $builder->keyId()->keyText('nickname', "用户昵称");
        foreach ($fields_list as $vt) {
     ...
     * @author RFly<>
     */
    public function message($page = 1, $r = 20)
    {
        $map['status'] = array('egt', 0);
        $messageList = D('Message')->where($map)->order("create_time desc")->page($page, $r)->select();
        $totalCount = D('Message')->count();
        foreach ($messageList as &$v) {
            if ($v['from_uid'] != 0) {
                $user = query_user(array('nickname'), $v['from_uid']);
                $v['from_user'] = $user['nickname'];
            }
            if ($v['to_uid'] != 0) {
                $user = query_user(array('nickname'), $v['to_uid']);
                $v['to_user'] = $user['nickname'];
            }
            if($v['is_read'] == 0)
                $v['is_read'] = '未读';
            else
        ...
        $builder = new AdminListBuilder();
        $builder->title("用户站内消息列表");
        $builder->meta_title = '用户站内消息';
        $builder->buttonDelete(U('changeMessageStatus', array('status' => '-1')))->setStatusUrl(U('changeMessageStatus'));
        $builder->keyId()->keyLink('title', "标题", 'User/message_details?id=###')->keyLinkByFlag('from_user', "发送者",'User/expandinfo_details?uid=###','from_uid')->keyLinkByFlag('to_user', '接收者','User/expandinfo_details?uid=###','to_uid')->keyTime("create_time", "创建时间")->keyText('is_read', '状态');
        //$builder->keyDoAction('User/field?id=###', '标为已读');
        $builder->data($messageList);
        $builder->pagination($totalCount, $r);
        $builder->display();
    }

    /**设置消息状态：删除=-1，禁用=0，启用=1
     * @param $status
     * @author RFly
     */
    public function changeMessageStatus($status)
    {
        $id = array_unique((array)I('ids', 0));
        if ($id[0] == 0) {
            $this->error('请选择要操作的数据!');
        ...
        $id = is_array($id) ? $id : explode(',', $id);
        D('Message')->where(array('id' => array('in', $id)))->setField('status', $status);
        if ($status == -1) {
            $this->success('删除成功');
        } else if ($status == 0) {
            $this->success('禁用成功');
    ...
    public function message_details($id = 0)
    {
        ...
        $map['status'] = array('egt', 0);
        //查询
        $message = D('Message')->where($map)->find();
        $builder = new AdminConfigBuilder();
        $builder->title("用户站内消息详情");
        $builder->meta_title = '用户站内消息详情';
        $builder->keyId()->keyReadOnly('title', "标题");
        foreach($message as $k => &$v){
            switch($k){
                case 'from_uid':
                    $user = query_user(array('nickname'), $v);
                    $v = $user['nickname'];
                    $builder->keyReadOnly($k,'发送者');
                    break;
                case 'to_uid':
                    $user = query_user(array('nickname'), $v);
                    $v = $user['nickname'];
                    $builder->keyReadOnly($k,'接收者');
                    break;
                case 'content':
                    $builder->keyEditor($k, '内容');
                    break;
            }
        }
