...
class FollowModel extends Model
{
    ...
    public function follow($uid)
    {
        ...
        D('Message')->sendMessage($uid, $user['username'] . ' 关注了你。', '粉丝数增加', $user['space_url'], is_login(), 2, null, 'follow_you');
    ...
    public function unfollow($uid)
    {
        ...
        $user = query_user(array('id', 'username', 'space_url'));
        //取消关注不需要消息
        D('Message')->sendMessage($uid, $user['username'] . '取消了对你的关注', '粉丝数减少', $user['space_url'], is_login(), 2, null, 'unfollow_you');
        return $this->where($follow)->delete();
    }

    public function getFans($uid, $page,$count,$fields,&$totalCount)
    {
        $map['follow_who'] = $uid;
        $fans = $this->where($map)->field('who_follow')->order('create_time desc')->page($page, $count)->select();
    ...
    public function getFollowing($uid, $page,$count,$fields,&$totalCount)
    {
        $map['who_follow'] = $uid;
        $fans = $this->where($map)->field('follow_who')->order('create_time desc')->page($page, $count)->select();
