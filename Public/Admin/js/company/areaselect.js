...
define(['jquery','areadata'],function () {
    function locationCard(settings) {
        ...
        var targetSelector=_options.targetSelector;
        var self = this;
        var pdiv, cdiv, xdiv;
        ...
        function showCityList(province) {
            ...
            //    };
            c_css = c_css || {
                    'top':cInput.outerHeight() + 'px',
                    'left': cInput.position().left + 'px'
                };
        ...
        function showProvinceList() {
            ...
            p_css = p_css || {
                    'top':pInput.outerHeight() + 'px',
                    'left': pInput.position().left + 'px'
                };
        ...
        function showCountyList() {
            ...
            hideProvinceList();
            hideCityList();
            if (!cur_p || cur_p.length == 0 || !cur_c || cur_c.length == 0) {
                return;
            }
        ...
        function createX(countys) {
            ...
            if (xdiv && xdiv.length > 0) {
                for (var i = 0; i < countys.length; i++) {
                    html += '<a class="a" href="javascript:void(0);" style="text-decoration: none !important;">' + countys[i] + '</a>';
                }
                xdiv.html(html);
                return xdiv;
            }
        ...
        function createC(citys) {
            ...
            if (cdiv && cdiv.length > 0) {
                for (var i = 0; i < citys.length; i++) {
                    html += '<a class="a" href="javascript:void(0);" style="text-decoration: none !important;">' + citys[i] + '</a>';
                }
                cdiv.html(html);
                return cdiv;
            }
        ...
        function createP() {
            ...
            for (var i = 0; i < p_list.length; i++) {
                html += '<a class="a" href="javascript:void(0);" style="text-decoration: none !important;">' + p_list[i] + '</a>';
            }
            html += '</div>';
            pdiv = $(html);
